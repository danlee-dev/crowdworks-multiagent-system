...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://49.50.128.6:8000';
          ...
          if (!message.saved_to_db) {
            ...
            try {
              console.log(`📤 백엔드로 전송할 메시지 데이터:`, {
                id: message.id,
                has_fullDataDict: !!message.fullDataDict,
                fullDataDict_keys: message.fullDataDict ? Object.keys(message.fullDataDict).length : 0,
          ...
          formattedConversation.messages = uniqueMessages.map(msg => {
            ...
            if (msg.type === 'assistant') {
              console.log(`🔍 Assistant 메시지 상태 정보:`, {
                ...
                has_full_data_dict: !!msg.full_data_dict,
                full_data_dict_keys: msg.full_data_dict ? Object.keys(msg.full_data_dict).length : 0,
