...
const PDFDownloadButton = ({ 
  ...
  // fullDataDict에서 참고문헌 추출
  const references = React.useMemo(() => {
    if (!fullDataDict || Object.keys(fullDataDict).length === 0) {
      return [];
    }

    const refs = [];
    Object.entries(fullDataDict).forEach(([key, data], index) => {
      if (data && data.content) {
        refs.push({
          number: index + 1,
          title: data.title || `출처 ${index + 1}`,
          content: data.content,
          url: data.url || data.source_url,
          source_type: data.source_type || data.source || '출처',
