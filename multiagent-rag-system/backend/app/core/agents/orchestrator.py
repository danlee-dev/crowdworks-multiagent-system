...
from ...services.search.search_tools import scrape_and_extract_content
...
분류 기준:
1. **chat**: 간단한 질문, 일반적인 대화, 답담, 그리고 간단한 Web Search나, vector db를 조회하여 답변할 수 있는 경우, 특정 링크/URL의 내용을 간단히 확인하고 싶은 경우
   - 예: "안녕하세요", "감사합니다", "간단한 설명 요청", "최근 ~ 시세 알려줘", "최근 이슈 Top 10이 뭐야?", "이 링크 내용이 뭐야?"

2. **task**: 복합적인 분석, 데이터 수집, 리포트 생성이 필요, 정확히는 여러 섹션에 걸친 보고서 생성이 필요한 질문일 경우 또는, 자세한 영양 정보와 같은 RDB를 조회 해야하는 질문일 경우, 특정 URL을 바탕으로 상세한 보고서나 전략 문서를 작성해달라는 경우, Graph DB를 거칠 필요가 있는 경우, 논문 검색이 필요한 경우
   - 예: "~를 분석해줘", "~에 대한 자료를 찾아줘", "보고서 작성", "이 링크 내용을 바탕으로 전략 보고서 작성해줘", "~에 대해 검색하고 상세한 분석 보고서 생성해줘"
...
class OrchestratorAgent:
    ...
    def _parse_docrels_from_graph_report(self, text: str):
        """
        neo4j_rag_tool의 문자열 리포트에서 '문서 관계 정보' 섹션을 추출해
    ...
    async def _probe_graph_data_locations(self, query: str) -> dict:
        ...
        try:
            ...
            return {"error": str(e), "has_isfrom": False, "has_nutrients": False, "has_docrels": False, "docrels": [], "raw_report": ""}
...
{prompt}"""
    ...
    async def generate_plan(self, state: StreamingAgentState) -> StreamingAgentState:
        ...
        # === NEW: 그래프 프리-체크 결과를 플래닝 프롬프트에 주입 ===
...
**3. graph_db_search (Neo4j) - 1순위 활용**
   - **데이터 종류**: **관계형(그래프) 데이터**. 노드: 품목(농산물/수산물/축산물), **Origin(원산지: city/region)**, **Nutrient(영양소)**, Entity(vector_db내 키워드).
     관계: `(Ingredient)-[:isFrom]->(Origin)`, `(Food)-[:hasNutrient]->(Nutrient)`. `(Entity)-[:relation]->(Entity)`.
   - **사용 시점**: **품목 ↔ 원산지**, **품목 ↔ 영양소**처럼 **엔티티 간 연결**이 핵심일 때. 지역·상태 조건을 얹은 **원산지/특산품 탐색**. 또는 vector_db 내 전역적인 키워드 관계 탐색이 필요할 때.
   - **특징**: 지식그래프 경로 탐색에 최적화. 키워드는 **품목명/지역명/영양소**로 간결히 표현하고, 질문은 **"A의 원산지", "A의 영양소", "지역 B의 특산품/원산지", "활어 A의 원산지"**처럼 **관계를 명시**할수록 정확도 상승.
...
**반영 규칙(강제)**
1) {gp_isfrom} 이면: **원산지/특산품/연결성** 관련 하위질문은 `graph_db_search`를 포함할 것.
2) {gp_nutr} 이면: **정량(함량/단위/순위/비교)**가 필요한 하위질문은 `rdb_search`를 포함하고, **연결(어떤 영양소가 붙는지)**만 볼 때는 `graph_db_search`를 보조로 사용할 것.
3) {gp_doc} 이면: **vector_db_search** 하위질문을 최소 1개 포함하되, 아래 docrels(source/target/type/doc)를 **키워드에 반영**하여 쿼리를 작성할 것.
...
"""
    ...
    """
        ...
        if len(full_data_context) > 40000:
            ...
            for i, res in enumerate(current_collected_data):
                source = getattr(res, 'source', 'Unknown')
                title = getattr(res, 'title', 'No Title')
                content = getattr(res, 'content', '')[:3000]  # 3000자로 제한
    ...
    **목표**: 차트 생성에 필요한 충분한 데이터 확보 (품질 중심 선택)
    ...
    async def execute_report_workflow(self, state: StreamingAgentState, run_manager=None) -> AsyncGenerator[str, None]:
        ...
        # 0. (NEW) 그래프 DB 프리-체크: 정보 위치 신호 생성 (환경변수로 제어)
        disable_precheck = os.environ.get("DISABLE_PRECHECK", "false").lower() == "true"
        
        if disable_precheck:
            # 프리-체크 비활성화: 기본값으로 설정
            yield self._create_status_event("PLANNING", "GRAPH_PROBE_DISABLED", "그래프 프리-체크 비활성화됨 (DISABLE_PRECHECK=true)")
            graph_probe = {
                "has_isfrom": False,
                "has_nutrients": False, 
                "has_docrels": False,
            ...
            print(f"🔴 PRECHECK DISABLED: DISABLE_PRECHECK={os.environ.get('DISABLE_PRECHECK')}")
        else:
            # 프리-체크 활성화: 실제 그래프 조회
            print(f"🟢 PRECHECK ENABLED: DISABLE_PRECHECK={os.environ.get('DISABLE_PRECHECK')}")
            ...
            if graph_probe.get("error"):
                ...
                msg = f"Pre-Check 완료 - 원산지 관계: {isfrom_status} / 영양성분 관계: {nutrient_status} / 문서 관계: {docrel_status}"
    ...
    **새 데이터**: {new_data_indexes}
    ...
    async def _log_chart_verification(self, query: str, section_title: str, section_description: str, section_data_list: List[SearchResult], chart_data: dict, state: dict):
        ...
        try:
            ...
            # 검증 로그 내용 생성
            kst = pytz.timezone('Asia/Seoul')
            timestamp = datetime.now(kst).strftime("%Y%m%d_%H%M%S_%f")[:-3]
    ...
    async def _log_section_verification(self, query: str, section_title: str, section_description: str, section_data_list: List[SearchResult], section_content: str, state: dict):
        ...
        try:
            ...
            # 섹션 인덱스 생성 (이미 추가된 후이므로 현재 길이가 실제 인덱스)
            section_index = len(state.get('accumulated_context', {}).get('generated_sections', []))
...
생성 시간: {datetime.now(pytz.timezone('Asia/Seoul')).strftime('%Y-%m-%d %H:%M:%S KST')}
타임스탬프: {timestamp}
...
"""
    ...
    def _get_query_log_dir(self, query: str, session_id: str, state: dict = None) -> str:
        """쿼리별 로그 디렉토리 경로 생성 (한국시간 기준, 시간순 정렬 가능)"""
        ...
        # 타임스탬프를 맨 앞에 (YYYYMMDD_HHMMSS 형식으로 정렬 가능하게)
        timestamp = kst_time.strftime("%Y%m%d_%H%M%S")

        # 쿼리 텍스트 정제 (30자로 제한하여 간결하게)
        sanitized_query = self._sanitize_query_for_folder_name(query, max_length=30)

        # 채팅방 제목 추출 (state에서 chatroom_title 또는 project_name 사용)
        chatroom_title = "채팅방"
        if state:
            # 채팅방 제목 추출 시도
            chatroom_info = state.get("chatroom_title") or state.get("chatroom_name")

            # 채팅방 제목이 없으면 프로젝트명으로 대체
            if not chatroom_info:
                chatroom_info = state.get("project_name") or state.get("project_title")

            # metadata에서도 확인
            if not chatroom_info and "metadata" in state:
                metadata = state["metadata"]
                chatroom_info = (metadata.get("chatroom_title") or
                                metadata.get("chatroom_name") or
                                metadata.get("project_name") or
                                metadata.get("project_title"))

            if chatroom_info:
                chatroom_title = self._sanitize_query_for_folder_name(chatroom_info, max_length=30)

        # 폴더명: 시간(한국기준)_쿼리(요약)_채팅방-제목
        folder_name = f"{timestamp}_{sanitized_query}_{chatroom_title}"
        ...
        print(f"📁 로그 디렉토리 생성 (KST): {query_dir}")
