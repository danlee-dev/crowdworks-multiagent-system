...
"""
    ...
    def _extract_params_with_llm(self, query: str, chosen_tables: List[str]) -> Dict[str, Any]:
        ...
        prompt = f"""
너는 SQL 파라미터 추출기다. 사용자 질문을 분석하여 RDB 검색 가능한 형태로 변환한다.

[핵심 변환 규칙]
1. 품목명 구체화:
   - "농축수산물" → ["사과", "배", "쌀", "감자"] (구체적 품목들로 분해)
   - "특산품" → ["감귤", "사과", "배"]
   - "농산물" → ["사과", "배", "쌀", "감자"]
   - "수산물" → ["고등어", "명태"] (KAMIS에 제한적)
   - "건강기능식품" → ["홍삼", "프로폴리스"]
   - 구체적 품목("사과", "감귤")은 그대로 유지

2. 테이블 구조:
   - 가격/시세: kamis_product_price_latest.item_name (사과, 배, 감귤, 쌀, 고등어, 전복 등)
   - 영양성분: foods."식품명" (사과, 쌀, 고등어 등) + 영양 테이블들 조인
...
반환 JSON (추상적 품목은 구체적 품목들로 변환):
{{
  "search_type": "price|nutrition|general",
  "items": ["구체적인 품목명들만 (사과, 배, 감귤 등)"],
  "regions": ["지역명 (없으면 [])"],
  "time_period": "recent|today|this_week|this_month",
  "specific_info": ["실제 컬럼에 존재하는 영양성분명만"],
  "search_intent": "사용자 의도 요약"
}}
...
class PostgreSQLRAG:
    ...
    def comprehensive_search(self, query: str) -> Dict[str, Any]:
        ...
        # 디버깅: 추출된 파라미터 상세 로그
        print(f">> 추출된 파라미터 상세:")
        print(f"   - search_type: {params.get('search_type')}")
        print(f"   - items: {params.get('items')}")
        print(f"   - regions: {params.get('regions')}")
        print(f"   - date_range: {params.get('date_range')}")
        print(f"   - specific_info: {params.get('specific_info')}")
