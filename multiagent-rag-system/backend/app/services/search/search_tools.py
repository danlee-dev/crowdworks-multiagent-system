    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "organic" in data and data["organic"]:
                for result in data["organic"][:10]:  # 상위 10개로 증가
                    link = result.get("link", "")
                    title = result.get("title", "No title")
                    snippet = result.get("snippet", "No snippet")

                    # link가 빈 문자열이 아닌지 확인 (None이나 빈 문자열 필터링)
                    if link and link.strip():  # 공백만 있는 경우도 제외
                        # http/https가 없으면 추가
                        ...
                        # link가 없는 결과도 로깅
                        session_print("WebSearch", f"링크 없는 결과 스킵: title={title[:30]}...")
                        
            # 결과가 없을 경우 더 자세한 디버깅 정보 출력
            if not results:
                session_print("WebSearch", f"검색 결과 없음. 전체 API 응답 키: {list(data.keys())}")
                session_print("WebSearch", f"organic 결과 수: {len(data.get('organic', []))}")
                if 'organic' in data and data['organic']:
                    session_print("WebSearch", f"첫 번째 organic 결과: {data['organic'][0]}")
                session_print("WebSearch", f"answerBox 존재: {'answerBox' in data}")
                session_print("WebSearch", f"API 응답 샘플: {json.dumps(data, ensure_ascii=False)[:500]}")
            ...
            session_print("WebSearch", f"API 응답 오류: status={response.status_code}")
            session_print("WebSearch", f"응답 내용: {response.text[:500]}")
            return f"API 오류: {response.status_code}, {response.text[:200]}"
    except requests.exceptions.Timeout:
        session_print("WebSearch", "요청 시간 초과 (10초)")
        return "웹 검색 시간 초과 - 다시 시도해주세요"
    except requests.exceptions.RequestException as e:
        session_print("WebSearch", f"네트워크 오류: {e}")
        return f"웹 검색 네트워크 오류: {str(e)}"
    except Exception as e:
        session_print("WebSearch", f"예기치 않은 오류: {e}")
    ...
    try:
        with sync_playwright() as p:
            ...
            context.add_cookies([
                {
                    'name': 'session_id',
                    'value': f'guest_{hash(url) % 100000}',
                    'domain': '.airport.kr',
                    'path': '/'
                },
                {
            ...
            print(f"  → 페이지 로딩 시도 중: {url}")
            
            # 다단계 접근 전략
            strategies = [
                {
                    "name": "direct_networkidle",
                    "wait_until": "networkidle",
                    "timeout": 30000,
                    "referer": None
                },
                {
                    "name": "with_google_referer",
                    "wait_until": "domcontentloaded", 
                    "timeout": 20000,
                    "referer": "https://www.google.com/"
                },
                {
                    "name": "with_search_referer",
                    "wait_until": "domcontentloaded",
                    "timeout": 20000, 
                    "referer": f"https://www.google.com/search?q={url}"
                },
                {
                    "name": "with_site_referer",
                    "wait_until": "load",
                    "timeout": 15000,
                    "referer": "https://parking.airport.kr/"
                },
                {
                    "name": "minimal_load",
                    "wait_until": "commit",
            ...
            for i, strategy in enumerate(strategies):
                try:
                    print(f"  → 시도 #{i+1}: {strategy['name']}")
                    
                    # Referer 헤더 설정
                    if strategy["referer"]:
                        page.set_extra_http_headers({
                            **page.extra_http_headers,
                    ...
                    print(f"  → HTTP 응답: {response.status if response else 'Unknown'}")
                    
                    if response and response.status == 200:
                        print(f"  ✓ 성공: {strategy['name']}")
                        success = True
                        break
                    elif response and response.status in [301, 302, 303, 307, 308]:
                        print(f"  → 리다이렉트 감지, 계속 진행")
                        success = True
                        break
                    else:
                        print(f"  → 실패 (상태: {response.status if response else 'No Response'})")
                        continue
            ...
            for loc in locators:
                try:
                    ...
                    if elements.count() > 0:
                        content = elements.first.inner_text(timeout=3000)
                        if len(content) > 100:
                            print(f"  ✓ '{loc}' 선택자에서 본문 발견 ({len(content)}자)")
                            break
                except Exception:
                    continue

            # 위에서 못 찾으면 body 전체를 사용
            if not content or len(content) < 50:
                try:
                    content = page.locator('body').inner_text(timeout=5000)
                    print(f"  ✓ body 선택자 사용 ({len(content)}자)")
                except:
                    # 최후의 수단: 페이지 전체 텍스트
                    content = page.inner_text('html')
                    print(f"  ✓ html 전체 텍스트 사용 ({len(content)}자)")

            browser.close()

            # 내용이 너무 적거나 접근 거부 메시지인 경우
            if not content or len(content) < 20:
                return "웹페이지에서 내용을 추출할 수 없었습니다."
            
            # 접근 거부 관련 키워드 확인
            reject_keywords = ['요청 허용 안함', '잘못된 요청', '접근 할 수 없습니다', 'Access Denied', '403', '404']
            if any(keyword in content for keyword in reject_keywords):
                return f"웹사이트 접근이 제한되었습니다. 추출된 내용: {content[:200]}..."
...
def vector_db_search(query: str, top_k = 20) -> List:
    ...
    try:
        ...
        for i, doc in enumerate(top_results):
            ...
            meta_data = doc.get('meta_data', doc.get('metadata', {}))
            title = meta_data.get('document_title', f'Document {i+1}')
            ...
            # 출처 정보 더 명확히 포함
            source_info = meta_data.get('document_link', 'Vector DB')
            page_number = meta_data.get('page_number', 'N/A')
