...
- **평가 방식**: AI Judge 활성화 (Gemini 2.5 Flash 기반 자동 평가)
- **평가 대상**: 5개 페르소나 × 3개 쿼리 = 총 15개 보고서
- **평가 모델**: Gemini 2.5 Flash (품질 및 환각 평가용)
...
| 평가 지표 | 가중치 | 측정 방법 | 측정 도구 |
|-----------|--------|-----------|-----------|
| **1. 작업 성공률** | 25% | 요구사항 충족도 (완료된 필수 요구사항 / 전체 필수 요구사항) | 자동 평가 |
| **2. 품질 점수** | 25% | AI Judge 평가 (사실 정확도 40% + 논리적 일관성 30% + 요구사항 부합도 30%) | Gemini 2.5 Flash |
| **3. 완성도** | 20% | 섹션 완성도 (생성된 마크다운 헤더 수 / 기대 섹션 수) | 자동 평가 |
| **4. 환각 점수** | 15% | 역점수 (10 - 환각 발생 건수 × 2), 인용 정확성 검증 | Gemini 2.5 Flash |
| **5. 효율성 점수** | 10% | 실행 시간 기반 (60초 이하: 10점, 120초 이상: 6.5점 이하) | 자동 측정 |
| **6. 출처 품질** | 5% | 출처 신뢰도 평균 (0.0~1.0) + 출처 다양성 | 자동 평가 |
| **7. 콘텐츠 메트릭** | 참고 | 단어 수, 섹션 수, 차트 수 (점수 미반영, 통계용) | 자동 측정 |
